	...
	return eg.Sequential(
		egbug.DebugFailure(
			shell.Op(
				...
				flutter.New("bash -c 'cd ios && rm -rf RetrovivedBind.framework RetrovivedBind.xcframework && mkdir -p RetrovivedBind.framework && xcrun clang -target arm64-apple-ios16.0 -isysroot \"$(xcrun --sdk iphoneos --show-sdk-path)\" -shared -o RetrovivedBind.framework/RetrovivedBind $(for f in *.a; do printf -- \"-Wl,-force_load,%s \" \"$f\"; done) -lc++ -lresolv -framework CoreFoundation -framework Security -Wl,-install_name,@rpath/RetrovivedBind.framework/RetrovivedBind && /usr/libexec/PlistBuddy -c \"Add :CFBundleExecutable string RetrovivedBind\" -c \"Add :CFBundleIdentifier string space.retrovibe.retrovibedbind\" -c \"Add :CFBundleInfoDictionaryVersion string 6.0\" -c \"Add :CFBundleName string RetrovivedBind\" -c \"Add :CFBundlePackageType string FMWK\" -c \"Add :CFBundleVersion string 1.0.0\" -c \"Add :CFBundleShortVersionString string 1.0.0\" -c \"Add :MinimumOSVersion string 16.0\" RetrovivedBind.framework/Info.plist && xcodebuild -create-xcframework -framework RetrovivedBind.framework -output RetrovivedBind.xcframework'"),
