	...
	err := eg.Perform(
		...
		eg.Parallel(
			shell.Op(
				...
				shell.Env().New("gh workflow run release.ios.yml --ref james.lawrence/egapplex-refactor").Attempts(3),
				shell.Env().New("gh workflow run release.macosx.yml --ref james.lawrence/egapplex-refactor").Attempts(3),
