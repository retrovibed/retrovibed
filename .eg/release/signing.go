...
func Keychain(base64key, keypassword string) eg.OpFn {
	return func(ctx context.Context, o eg.Op) error {
		...
		return shell.Run(
			...
			// env.New("curl -fLo ${APPLE_INTERMEDIATE_CERT} $( (openssl pkcs12 -legacy -in ${APPLE_SIGNING_KEY_PATH} -nokeys -passin pass:${APPLE_SIGNING_KEY_PASSWORD} 2>/dev/null || openssl pkcs12 -in ${APPLE_SIGNING_KEY_PATH} -nokeys -passin pass:${APPLE_SIGNING_KEY_PASSWORD}) | openssl x509 -noout -text | grep \"CA Issuers - URI:\" | cut -d':' -f2- | xargs)"),
