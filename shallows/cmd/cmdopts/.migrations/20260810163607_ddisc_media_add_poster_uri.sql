...
CREATE TABLE ddisc_media (
  ...
  contentmime STRING NOT NULL DEFAULT 'application/x-bittorrent' CHECK (contentmime <> ''),
  poster_uri STRING NOT NULL DEFAULT '',
...
COMMENT ON COLUMN ddisc_media.acquisition_state IS 'mirrors ddisc.AcquisitionState / ddisc.discovery.proto AcquisitionState ordinals: 0 unknown, 1 ephemeral (search result, not persisted), 2 available (persisted, not downloading), 3 downloading, 4 completed - see shallows/ddisc.DiscoveredOptionAcquisitionState';
COMMENT ON COLUMN ddisc_media.contentmime IS 'how the uri is fetched, not the media mimetype: mimex.Bittorrent for magnet/.torrent sources, mimex.HTTP for plain http(s) sources - see shallows/ddisc.DiscoveredOptionContentMime';
COMMENT ON COLUMN ddisc_media.poster_uri IS 'thumbnail/poster image url surfaced by whatever discovered this candidate (e.g. PeerTube); also TOFU-recorded onto library_known_media.poster_path once a known-media id resolves - see shallows/ddisc.KnownMediaTOFU';
INSERT INTO ddisc_media (
  id, infohash, uri, created_at, updated_at, next_check_at, attempts, hidden_at, tombstoned_at, released_at,
  title, source, "collation", description, mimetype, contentmime, poster_uri, bytes, video_resolution, video_runtime, audio_bitrate,
  audio_default_locale, subtitles_default_locale, known_media_id, sync_uid, "partition", category, adult,
  private, health, policy_rank, policy_rejection, acquisition_state
)
SELECT
  id, infohash, uri, created_at, updated_at, next_check_at, attempts, hidden_at, tombstoned_at, released_at,
  title, source, "collation", description, mimetype, contentmime, '',
  bytes, video_resolution, video_runtime, audio_bitrate,
  audio_default_locale, subtitles_default_locale, known_media_id, sync_uid, "partition", category, adult,
  private, health, policy_rank, policy_rejection, acquisition_state
...
INSERT INTO ddisc_media (
  id, infohash, uri, created_at, updated_at, next_check_at, attempts, hidden_at, tombstoned_at, released_at,
  title, source, "collation", description, mimetype, contentmime, bytes, video_resolution, video_runtime, audio_bitrate,
  audio_default_locale, subtitles_default_locale, known_media_id, sync_uid, "partition", category, adult,
  private, health, policy_rank, policy_rejection, acquisition_state
)
SELECT
