...
func TestSearchPluginImport(t *testing.T) {
	...
	setup := func(t *testing.T, filename string, content []byte, options ...func(*tracking.Metadata)) (sqlx.Queryer, fsx.Virtual, storage.ClientImpl, string, tracking.Metadata) {
		t.Helper()
		...
		lmd := tracking.NewMetadata(
			...
			append([]func(*tracking.Metadata){
				...
				tracking.MetadataOptionMimetype(mimex.RetrovibedDiscoverySearch),
	...
	t.Run("imports a valid wasm module and marks it imported", func(t *testing.T) {
		...
		q, tvfs, tstore, plugindir, lmd := setup(t, "plugin.wasm", content, tracking.MetadataOptionCompleted)

		require.NoError(t, daemons.SearchPluginImport(t.Context(), q, plugindir, tvfs, tstore))

		written, err := os.ReadFile(filepath.Join(plugindir, "plugin.wasm"))
		require.NoError(t, err)
		...
		require.Equal(t, lmd.Description, "plugin.wasm")
	})

	t.Run("appends .wasm when the description is missing the suffix", func(t *testing.T) {
		content := wasmModule(t, 4096)
		q, tvfs, tstore, plugindir, _ := setup(t, "plugin-no-suffix", content, tracking.MetadataOptionCompleted)

		require.NoError(t, daemons.SearchPluginImport(t.Context(), q, plugindir, tvfs, tstore))

		written, err := os.ReadFile(filepath.Join(plugindir, "plugin-no-suffix.wasm"))
		require.NoError(t, err)
		require.Equal(t, content, written)
	})
	...
	t.Run("tombstones and does not copy a payload that isn't a valid wasm module", func(t *testing.T) {
		...
		_, err := os.Stat(filepath.Join(plugindir, "plugin.wasm"))
	...
	t.Run("skips a plugin torrent not yet completed", func(t *testing.T) {
		content := wasmModule(t, 4096)
		q, tvfs, tstore, plugindir, _ := setup(t, "plugin.wasm", content)
