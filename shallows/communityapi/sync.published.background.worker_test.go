...
func TestSyncPublishedBackgroundWorker(t *testing.T) {
	t.Run("skips content without archive", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, fsx.DirVirtual(t.TempDir()), fsx.DirVirtual(t.TempDir()))
	...
	t.Run("generates torrent for content with empty magnet uri", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, fsx.DirVirtual(mediaDir), fsx.DirVirtual(torrentDir))
		encoded, err := jsonx.Marshal(langx.Clone(pc, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
	...
	t.Run("uses existing torrent when torrent_id is set", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, fsx.DirVirtual(t.TempDir()), fsx.DirVirtual(torrentDir))
		encoded, err := jsonx.Marshal(langx.Clone(pc, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
	...
	t.Run("marks existing torrent as completed and seeding", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, fsx.DirVirtual(t.TempDir()), tvfs)
		encoded, err := jsonx.Marshal(langx.Clone(pc, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
	...
	t.Run("same content produces same magnet uri", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, mvfs, tvfs)
		encoded, err := jsonx.Marshal(langx.Clone(pc, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
		...
		worker = NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, nil, mvfs, tvfs)
		encoded, err = jsonx.Marshal(langx.Clone(afterFirst, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
	...
	t.Run("archives content with nil archive_id then publishes to deeppool", func(t *testing.T) {
		...
		worker := NewSyncPublishedBackgroundWorker(q, http.DefaultClient, mock, feeds, publishplugin.Unimplemented{}, archiver, fsx.DirVirtual(mediaDir), fsx.DirVirtual(torrentDir))
		encoded, err := jsonx.Marshal(langx.Clone(pc, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded))
	...
	t.Run("does not leak a still-pending sibling into a feed regenerated mid-sync", func(t *testing.T) {
		...
		// pc2 is processed first so it is mid-flight, still pending, when
		// pc1 completes and regenerates the feed.
		encoded2, err := jsonx.Marshal(langx.Clone(pc2, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.Error(t, worker.Message(ctx, encoded2))

		encoded1, err := jsonx.Marshal(langx.Clone(pc1, timex.JSONSafeEncodeOption))
		require.NoError(t, err)
		require.NoError(t, worker.Message(ctx, encoded1))
