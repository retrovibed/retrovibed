...
// ContextBufferPoolBytes - convience pool that uses a 1024 byte allocation buffer pool.
func ContextBufferPoolBytes(bytesize int) func(http.Handler) http.Handler {
	return ContextBufferPool(NewBufferPool(bytesize))
}
