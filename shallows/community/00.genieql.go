...
func PublishedContentInsertWithDefaults(
	...
	gql.Into("published_content").Default("id", "published_at", "created_at", "updated_at").Conflict("ON CONFLICT (community_id, library_id) DO UPDATE SET magnet_uri = EXCLUDED.magnet_uri, known_media_id = EXCLUDED.known_media_id, publish_mode = EXCLUDED.publish_mode, oauth_google_id = EXCLUDED.oauth_google_id, published_at = DEFAULT, updated_at = DEFAULT")
}
