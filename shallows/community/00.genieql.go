...
func PublishedContentInsertWithDefaults(
	...
	gql.Into("published_content").Default("id", "created_at", "updated_at", "tombstoned_at").Conflict("ON CONFLICT (community_id, library_id) DO UPDATE SET magnet_uri = COALESCE(NULLIF(EXCLUDED.magnet_uri, ''), published_content.magnet_uri), known_media_id = EXCLUDED.known_media_id, publish_mode = EXCLUDED.publish_mode, oauth_google_id = EXCLUDED.oauth_google_id, bytes = EXCLUDED.bytes, published_at = COALESCE(NULLIF(published_at, 'infinity'), EXCLUDED.published_at), tombstoned_at = DEFAULT, updated_at = DEFAULT")
}
...
func CommunityInsertWithDefaults(
	...
	gql.Into("community").Default("created_at", "updated_at", "sync_feed_at", "subscribed_at", "sync_cursor_published_content").Conflict("ON CONFLICT (id) DO UPDATE SET updated_at = DEFAULT, account_id = EXCLUDED.account_id, mimetype = EXCLUDED.mimetype, domain = EXCLUDED.domain, description = EXCLUDED.description, entropy = EXCLUDED.entropy, bytes = EXCLUDED.bytes, default_publish_mode = EXCLUDED.default_publish_mode, hidden = EXCLUDED.hidden, url = EXCLUDED.url, adult = EXCLUDED.adult, default_ttl = EXCLUDED.default_ttl, default_language = EXCLUDED.default_language, auto_download = EXCLUDED.auto_download, next_sync_at = EXCLUDED.next_sync_at, sync_cursor_published_content = GREATEST(sync_cursor_published_content, EXCLUDED.sync_cursor_published_content)")
}
...
func CommunityUpdateNextSyncAt(
	gql genieql.Insert,
	pattern func(ctx context.Context, q sqlx.Queryer, c Community) NewCommunityScannerStaticRow,
) {
	gql.Into("community").Default("created_at", "updated_at", "auto_download", "account_id", "sync_cursor_published_content", "subscribed_at").Conflict("ON CONFLICT (id) DO UPDATE SET updated_at = DEFAULT, next_sync_at = EXCLUDED.next_sync_at")
}
...
func CommunityUpsertAutoDownload(
	...
	gql.Into("community").Default("created_at", "updated_at", "sync_feed_at", "next_sync_at", "account_id", "sync_cursor_published_content", "subscribed_at").Conflict("ON CONFLICT (id) DO UPDATE SET updated_at = DEFAULT, auto_download = EXCLUDED.auto_download")
}
...
func CommunityRequestFeedSync(
	...
	gql.Into("community").Default("created_at", "updated_at", "next_sync_at", "auto_download", "account_id", "sync_cursor_published_content", "subscribed_at").Conflict("ON CONFLICT (id) DO UPDATE SET updated_at = DEFAULT, sync_feed_at = EXCLUDED.sync_feed_at")
}
