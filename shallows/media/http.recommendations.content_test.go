...
func TestRecommendationsContent(t *testing.T) {
	t.Run("finds recommendation by content id", func(t *testing.T) {
		var (
			p     meta.Profile
			authz meta.Authz
		)
		...
		resp, req, err := httptestx.BuildRequestBytes(
			http.MethodGet, fmt.Sprintf("/content/%s", known.UID), nil,
	...
	t.Run("unknown content id returns 404", func(t *testing.T) {
		var (
			p     meta.Profile
			authz meta.Authz
		)
		...
		resp, req, err := httptestx.BuildRequestBytes(
			http.MethodGet, fmt.Sprintf("/content/%s", uuidx.WithSuffix(999)), nil,
			httptestx.RequestOptionAuthorization(httpauthtest.UnsafeClaimsToken(claims, httpauthtest.UnsafeJWTSecretSource)),
		)
		require.NoError(t, err)
		routes.ServeHTTP(resp, req)
