...
func TestDiscoveryDelete(t *testing.T) {
	t.Run("deletes discovery entry successfully", func(t *testing.T) {
		var expected ddiscapi.DiscoveryDeleteResponse
		require.NoError(t, testx.Fake(&expected))

		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, http.MethodDelete, r.Method)
			require.Equal(t, "/ddisc/discovery/some-id", r.URL.Path)
		...
		_, err := ddiscapi.DiscoveryDelete(t.Context(), c, "example.com", "some-id")
