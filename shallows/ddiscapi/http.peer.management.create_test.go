...
func TestHTTPPeerManagementCreate(t *testing.T) {
	var (
		p      meta.Profile
		v      meta.Authz
		peer   tracking.Peer
		result ddiscapi.PeerCreateResponse
	)
	...
	body := testx.Must(json.Marshal(ddiscapi.PeerCreateRequest{
		Peer: encoded,
	}))(t)
	...
	require.Equal(t, result.Peer.Infohash, peer.Peer)
	require.Equal(t, result.Peer.Description, peer.Description)
	require.Equal(t, result.Peer.Partition, peer.DdiscPartition)
	require.Equal(t, result.Peer.Syncoffset, peer.DdiscSyncoffset)

	require.NoError(t, tracking.PeerFindByID(t.Context(), q, result.Peer.Id).Scan(&peer))
	require.EqualValues(t, timex.Inf(), peer.TombstonedAt, "manually created peers should have a infinite life span")
}
