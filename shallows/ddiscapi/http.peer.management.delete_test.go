...
func TestHTTPPeerManagementDelete(t *testing.T) {
	t.Run("successfully delete", func(t *testing.T) {
		var (
			p      meta.Profile
			v      meta.Authz
			peer   tracking.Peer
			result ddiscapi.PeerDeleteResponse
		)
		...
		resp, req, err := httptestx.BuildRequestBytes(http.MethodDelete, fmt.Sprintf("/%s", peer.ID), nil, httptestx.RequestOptionAuthorization(token))
		require.NoError(t, err)

		require.NoError(t, tracking.PeerFindByID(ctx, q, peer.ID).Scan(&peer))
