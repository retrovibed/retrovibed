...
func (t *HTTPSSHOauth2) token(w http.ResponseWriter, req *http.Request) {
	...
	tmptoken := func() error {
		...
		return httpx.WriteJSON(w, httpx.GetBuffer(req), tokenJSON{
			AccessToken: ast,
			TokenType:   "BEARER",
	...
	if uuid.FromStringOrNil(iden.ProfileID).String() == uuid.Nil.String() {
		// in this case we only send an access token for registration.
		log.Println("no profile associated with public key", iden.ID)
		errorsx.Log(tmptoken())
		return
	}
	...
	if ts := time.Now(); p.DisabledPendingApprovalAt.Before(ts) || p.DisabledPendingApprovalAt.Equal(ts) {
		log.Println("profile is pending approval", iden.ProfileID)
		errorsx.Log(tmptoken())
		return
	}
