...
func TestHTTPUserManagementUpdate(t *testing.T) {
	t.Run("should update the display name", func(t *testing.T) {
		var (
			p      meta.Profile
			v      meta.Authz
			result metaapi.ProfileUpdateResponse
		)
		...
		b, err := json.Marshal(&metaapi.ProfileUpdateRequest{
			Profile: encoded,
		})
		require.NoError(t, err)
		...
		resp, req, err := httptestx.BuildRequestContext(ctx, http.MethodPatch, fmt.Sprintf("/u12t/%s", mut.ID), bytes.NewReader(b), httptestx.RequestOptionAuthorization(token))
		require.NoError(t, err)
		...
		require.Equal(t, "newusername", result.Profile.Display)
	})

	t.Run("should update the profile", func(t *testing.T) {
		var (
			p      meta.Profile
			v      meta.Authz
		...
		require.Equal(t, timex.Inf(), p.DisabledManuallyAt)
		mut := p
