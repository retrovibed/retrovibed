// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        v3.21.12
// source: wireguard/meta.wireguard.proto

package metaapi

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type WireguardNettype int32

const (
	WireguardNettype_UNSPECIFIED  WireguardNettype = 0
	WireguardNettype_DISTRIBUTION WireguardNettype = 1
	WireguardNettype_SOCIAL       WireguardNettype = 2
)

// Enum value maps for WireguardNettype.
var (
	WireguardNettype_name = map[int32]string{
		0: "UNSPECIFIED",
		1: "DISTRIBUTION",
		2: "SOCIAL",
	}
	WireguardNettype_value = map[string]int32{
		"UNSPECIFIED":  0,
		"DISTRIBUTION": 1,
		"SOCIAL":       2,
	}
)

func (x WireguardNettype) Enum() *WireguardNettype {
	p := new(WireguardNettype)
	*p = x
	return p
}

func (x WireguardNettype) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (WireguardNettype) Descriptor() protoreflect.EnumDescriptor {
	return file_wireguard_meta_wireguard_proto_enumTypes[0].Descriptor()
}

func (WireguardNettype) Type() protoreflect.EnumType {
	return &file_wireguard_meta_wireguard_proto_enumTypes[0]
}

func (x WireguardNettype) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use WireguardNettype.Descriptor instead.
func (WireguardNettype) EnumDescriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{0}
}

type Wireguard struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	Id                 string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	CreatedAt          string                 `protobuf:"bytes,2,opt,name=created_at,proto3" json:"created_at,omitempty"`
	UpdatedAt          string                 `protobuf:"bytes,3,opt,name=updated_at,proto3" json:"updated_at,omitempty"`
	Description        string                 `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	Nettype            WireguardNettype       `protobuf:"varint,5,opt,name=nettype,proto3,enum=meta.WireguardNettype" json:"nettype,omitempty"`
	Port               uint32                 `protobuf:"varint,6,opt,name=port,proto3" json:"port,omitempty"`
	RateLimitDns       uint32                 `protobuf:"varint,7,opt,name=rate_limit_dns,proto3" json:"rate_limit_dns,omitempty"`
	MaximumConnections uint64                 `protobuf:"varint,8,opt,name=maximum_connections,proto3" json:"maximum_connections,omitempty"`
	RateLimitOutbound  uint32                 `protobuf:"varint,9,opt,name=rate_limit_outbound,proto3" json:"rate_limit_outbound,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *Wireguard) Reset() {
	*x = Wireguard{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Wireguard) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Wireguard) ProtoMessage() {}

func (x *Wireguard) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Wireguard.ProtoReflect.Descriptor instead.
func (*Wireguard) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{0}
}

func (x *Wireguard) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Wireguard) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *Wireguard) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *Wireguard) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Wireguard) GetNettype() WireguardNettype {
	if x != nil {
		return x.Nettype
	}
	return WireguardNettype_UNSPECIFIED
}

func (x *Wireguard) GetPort() uint32 {
	if x != nil {
		return x.Port
	}
	return 0
}

func (x *Wireguard) GetRateLimitDns() uint32 {
	if x != nil {
		return x.RateLimitDns
	}
	return 0
}

func (x *Wireguard) GetMaximumConnections() uint64 {
	if x != nil {
		return x.MaximumConnections
	}
	return 0
}

func (x *Wireguard) GetRateLimitOutbound() uint32 {
	if x != nil {
		return x.RateLimitOutbound
	}
	return 0
}

type WireguardSearchRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Query         string                 `protobuf:"bytes,1,opt,name=query,proto3" json:"query,omitempty"`
	Offset        uint64                 `protobuf:"varint,2,opt,name=offset,proto3" json:"offset,omitempty"`
	Limit         uint64                 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardSearchRequest) Reset() {
	*x = WireguardSearchRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardSearchRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardSearchRequest) ProtoMessage() {}

func (x *WireguardSearchRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardSearchRequest.ProtoReflect.Descriptor instead.
func (*WireguardSearchRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{1}
}

func (x *WireguardSearchRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *WireguardSearchRequest) GetOffset() uint64 {
	if x != nil {
		return x.Offset
	}
	return 0
}

func (x *WireguardSearchRequest) GetLimit() uint64 {
	if x != nil {
		return x.Limit
	}
	return 0
}

type WireguardSearchResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Next          *WireguardSearchRequest `protobuf:"bytes,1,opt,name=next,proto3" json:"next,omitempty"`
	Items         []*Wireguard            `protobuf:"bytes,2,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardSearchResponse) Reset() {
	*x = WireguardSearchResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardSearchResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardSearchResponse) ProtoMessage() {}

func (x *WireguardSearchResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardSearchResponse.ProtoReflect.Descriptor instead.
func (*WireguardSearchResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{2}
}

func (x *WireguardSearchResponse) GetNext() *WireguardSearchRequest {
	if x != nil {
		return x.Next
	}
	return nil
}

func (x *WireguardSearchResponse) GetItems() []*Wireguard {
	if x != nil {
		return x.Items
	}
	return nil
}

type WireguardUpdateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardUpdateRequest) Reset() {
	*x = WireguardUpdateRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardUpdateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardUpdateRequest) ProtoMessage() {}

func (x *WireguardUpdateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardUpdateRequest.ProtoReflect.Descriptor instead.
func (*WireguardUpdateRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{3}
}

func (x *WireguardUpdateRequest) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

type WireguardUpdateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardUpdateResponse) Reset() {
	*x = WireguardUpdateResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardUpdateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardUpdateResponse) ProtoMessage() {}

func (x *WireguardUpdateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardUpdateResponse.ProtoReflect.Descriptor instead.
func (*WireguardUpdateResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{4}
}

func (x *WireguardUpdateResponse) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

type WireguardTouchRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Nettype       WireguardNettype       `protobuf:"varint,1,opt,name=nettype,proto3,enum=meta.WireguardNettype" json:"nettype,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardTouchRequest) Reset() {
	*x = WireguardTouchRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardTouchRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardTouchRequest) ProtoMessage() {}

func (x *WireguardTouchRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardTouchRequest.ProtoReflect.Descriptor instead.
func (*WireguardTouchRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{5}
}

func (x *WireguardTouchRequest) GetNettype() WireguardNettype {
	if x != nil {
		return x.Nettype
	}
	return WireguardNettype_UNSPECIFIED
}

type WireguardTouchResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardTouchResponse) Reset() {
	*x = WireguardTouchResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardTouchResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardTouchResponse) ProtoMessage() {}

func (x *WireguardTouchResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardTouchResponse.ProtoReflect.Descriptor instead.
func (*WireguardTouchResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{6}
}

func (x *WireguardTouchResponse) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

type WireguardUploadRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardUploadRequest) Reset() {
	*x = WireguardUploadRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardUploadRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardUploadRequest) ProtoMessage() {}

func (x *WireguardUploadRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardUploadRequest.ProtoReflect.Descriptor instead.
func (*WireguardUploadRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{7}
}

type WireguardUploadResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardUploadResponse) Reset() {
	*x = WireguardUploadResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardUploadResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardUploadResponse) ProtoMessage() {}

func (x *WireguardUploadResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardUploadResponse.ProtoReflect.Descriptor instead.
func (*WireguardUploadResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{8}
}

func (x *WireguardUploadResponse) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

type WireguardCurrentRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Nettype       WireguardNettype       `protobuf:"varint,1,opt,name=nettype,proto3,enum=meta.WireguardNettype" json:"nettype,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardCurrentRequest) Reset() {
	*x = WireguardCurrentRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardCurrentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardCurrentRequest) ProtoMessage() {}

func (x *WireguardCurrentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardCurrentRequest.ProtoReflect.Descriptor instead.
func (*WireguardCurrentRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{9}
}

func (x *WireguardCurrentRequest) GetNettype() WireguardNettype {
	if x != nil {
		return x.Nettype
	}
	return WireguardNettype_UNSPECIFIED
}

type WireguardCurrentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	Online        bool                   `protobuf:"varint,2,opt,name=online,proto3" json:"online,omitempty"`
	Ip            string                 `protobuf:"bytes,3,opt,name=ip,proto3" json:"ip,omitempty"`
	Ip4           string                 `protobuf:"bytes,4,opt,name=ip4,proto3" json:"ip4,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardCurrentResponse) Reset() {
	*x = WireguardCurrentResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardCurrentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardCurrentResponse) ProtoMessage() {}

func (x *WireguardCurrentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardCurrentResponse.ProtoReflect.Descriptor instead.
func (*WireguardCurrentResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{10}
}

func (x *WireguardCurrentResponse) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

func (x *WireguardCurrentResponse) GetOnline() bool {
	if x != nil {
		return x.Online
	}
	return false
}

func (x *WireguardCurrentResponse) GetIp() string {
	if x != nil {
		return x.Ip
	}
	return ""
}

func (x *WireguardCurrentResponse) GetIp4() string {
	if x != nil {
		return x.Ip4
	}
	return ""
}

type WireguardDeleteRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardDeleteRequest) Reset() {
	*x = WireguardDeleteRequest{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardDeleteRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardDeleteRequest) ProtoMessage() {}

func (x *WireguardDeleteRequest) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardDeleteRequest.ProtoReflect.Descriptor instead.
func (*WireguardDeleteRequest) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{11}
}

type WireguardDeleteResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Wireguard     *Wireguard             `protobuf:"bytes,1,opt,name=Wireguard,json=wireguard,proto3" json:"Wireguard,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WireguardDeleteResponse) Reset() {
	*x = WireguardDeleteResponse{}
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WireguardDeleteResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WireguardDeleteResponse) ProtoMessage() {}

func (x *WireguardDeleteResponse) ProtoReflect() protoreflect.Message {
	mi := &file_wireguard_meta_wireguard_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WireguardDeleteResponse.ProtoReflect.Descriptor instead.
func (*WireguardDeleteResponse) Descriptor() ([]byte, []int) {
	return file_wireguard_meta_wireguard_proto_rawDescGZIP(), []int{12}
}

func (x *WireguardDeleteResponse) GetWireguard() *Wireguard {
	if x != nil {
		return x.Wireguard
	}
	return nil
}

var File_wireguard_meta_wireguard_proto protoreflect.FileDescriptor

const file_wireguard_meta_wireguard_proto_rawDesc = "" +
	"\n" +
	"\x1ewireguard/meta.wireguard.proto\x12\x04meta\"\xcf\x02\n" +
	"\tWireguard\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x1e\n" +
	"\n" +
	"created_at\x18\x02 \x01(\tR\n" +
	"created_at\x12\x1e\n" +
	"\n" +
	"updated_at\x18\x03 \x01(\tR\n" +
	"updated_at\x12 \n" +
	"\vdescription\x18\x04 \x01(\tR\vdescription\x120\n" +
	"\anettype\x18\x05 \x01(\x0e2\x16.meta.WireguardNettypeR\anettype\x12\x12\n" +
	"\x04port\x18\x06 \x01(\rR\x04port\x12&\n" +
	"\x0erate_limit_dns\x18\a \x01(\rR\x0erate_limit_dns\x120\n" +
	"\x13maximum_connections\x18\b \x01(\x04R\x13maximum_connections\x120\n" +
	"\x13rate_limit_outbound\x18\t \x01(\rR\x13rate_limit_outbound\"\\\n" +
	"\x16WireguardSearchRequest\x12\x14\n" +
	"\x05query\x18\x01 \x01(\tR\x05query\x12\x16\n" +
	"\x06offset\x18\x02 \x01(\x04R\x06offset\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x04R\x05limit\"r\n" +
	"\x17WireguardSearchResponse\x120\n" +
	"\x04next\x18\x01 \x01(\v2\x1c.meta.WireguardSearchRequestR\x04next\x12%\n" +
	"\x05items\x18\x02 \x03(\v2\x0f.meta.WireguardR\x05items\"G\n" +
	"\x16WireguardUpdateRequest\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard\"H\n" +
	"\x17WireguardUpdateResponse\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard\"I\n" +
	"\x15WireguardTouchRequest\x120\n" +
	"\anettype\x18\x01 \x01(\x0e2\x16.meta.WireguardNettypeR\anettype\"G\n" +
	"\x16WireguardTouchResponse\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard\"\x18\n" +
	"\x16WireguardUploadRequest\"H\n" +
	"\x17WireguardUploadResponse\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard\"K\n" +
	"\x17WireguardCurrentRequest\x120\n" +
	"\anettype\x18\x01 \x01(\x0e2\x16.meta.WireguardNettypeR\anettype\"\x83\x01\n" +
	"\x18WireguardCurrentResponse\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard\x12\x16\n" +
	"\x06online\x18\x02 \x01(\bR\x06online\x12\x0e\n" +
	"\x02ip\x18\x03 \x01(\tR\x02ip\x12\x10\n" +
	"\x03ip4\x18\x04 \x01(\tR\x03ip4\"\x18\n" +
	"\x16WireguardDeleteRequest\"H\n" +
	"\x17WireguardDeleteResponse\x12-\n" +
	"\tWireguard\x18\x01 \x01(\v2\x0f.meta.WireguardR\twireguard*A\n" +
	"\x10WireguardNettype\x12\x0f\n" +
	"\vUNSPECIFIED\x10\x00\x12\x10\n" +
	"\fDISTRIBUTION\x10\x01\x12\n" +
	"\n" +
	"\x06SOCIAL\x10\x02b\x06proto3"

var (
	file_wireguard_meta_wireguard_proto_rawDescOnce sync.Once
	file_wireguard_meta_wireguard_proto_rawDescData []byte
)

func file_wireguard_meta_wireguard_proto_rawDescGZIP() []byte {
	file_wireguard_meta_wireguard_proto_rawDescOnce.Do(func() {
		file_wireguard_meta_wireguard_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_wireguard_meta_wireguard_proto_rawDesc), len(file_wireguard_meta_wireguard_proto_rawDesc)))
	})
	return file_wireguard_meta_wireguard_proto_rawDescData
}

var file_wireguard_meta_wireguard_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_wireguard_meta_wireguard_proto_msgTypes = make([]protoimpl.MessageInfo, 13)
var file_wireguard_meta_wireguard_proto_goTypes = []any{
	(WireguardNettype)(0),            // 0: meta.WireguardNettype
	(*Wireguard)(nil),                // 1: meta.Wireguard
	(*WireguardSearchRequest)(nil),   // 2: meta.WireguardSearchRequest
	(*WireguardSearchResponse)(nil),  // 3: meta.WireguardSearchResponse
	(*WireguardUpdateRequest)(nil),   // 4: meta.WireguardUpdateRequest
	(*WireguardUpdateResponse)(nil),  // 5: meta.WireguardUpdateResponse
	(*WireguardTouchRequest)(nil),    // 6: meta.WireguardTouchRequest
	(*WireguardTouchResponse)(nil),   // 7: meta.WireguardTouchResponse
	(*WireguardUploadRequest)(nil),   // 8: meta.WireguardUploadRequest
	(*WireguardUploadResponse)(nil),  // 9: meta.WireguardUploadResponse
	(*WireguardCurrentRequest)(nil),  // 10: meta.WireguardCurrentRequest
	(*WireguardCurrentResponse)(nil), // 11: meta.WireguardCurrentResponse
	(*WireguardDeleteRequest)(nil),   // 12: meta.WireguardDeleteRequest
	(*WireguardDeleteResponse)(nil),  // 13: meta.WireguardDeleteResponse
}
var file_wireguard_meta_wireguard_proto_depIdxs = []int32{
	0,  // 0: meta.Wireguard.nettype:type_name -> meta.WireguardNettype
	2,  // 1: meta.WireguardSearchResponse.next:type_name -> meta.WireguardSearchRequest
	1,  // 2: meta.WireguardSearchResponse.items:type_name -> meta.Wireguard
	1,  // 3: meta.WireguardUpdateRequest.Wireguard:type_name -> meta.Wireguard
	1,  // 4: meta.WireguardUpdateResponse.Wireguard:type_name -> meta.Wireguard
	0,  // 5: meta.WireguardTouchRequest.nettype:type_name -> meta.WireguardNettype
	1,  // 6: meta.WireguardTouchResponse.Wireguard:type_name -> meta.Wireguard
	1,  // 7: meta.WireguardUploadResponse.Wireguard:type_name -> meta.Wireguard
	0,  // 8: meta.WireguardCurrentRequest.nettype:type_name -> meta.WireguardNettype
	1,  // 9: meta.WireguardCurrentResponse.Wireguard:type_name -> meta.Wireguard
	1,  // 10: meta.WireguardDeleteResponse.Wireguard:type_name -> meta.Wireguard
	11, // [11:11] is the sub-list for method output_type
	11, // [11:11] is the sub-list for method input_type
	11, // [11:11] is the sub-list for extension type_name
	11, // [11:11] is the sub-list for extension extendee
	0,  // [0:11] is the sub-list for field type_name
}

func init() { file_wireguard_meta_wireguard_proto_init() }
func file_wireguard_meta_wireguard_proto_init() {
	if File_wireguard_meta_wireguard_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_wireguard_meta_wireguard_proto_rawDesc), len(file_wireguard_meta_wireguard_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   13,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_wireguard_meta_wireguard_proto_goTypes,
		DependencyIndexes: file_wireguard_meta_wireguard_proto_depIdxs,
		EnumInfos:         file_wireguard_meta_wireguard_proto_enumTypes,
		MessageInfos:      file_wireguard_meta_wireguard_proto_msgTypes,
	}.Build()
	File_wireguard_meta_wireguard_proto = out.File
	file_wireguard_meta_wireguard_proto_goTypes = nil
	file_wireguard_meta_wireguard_proto_depIdxs = nil
}
