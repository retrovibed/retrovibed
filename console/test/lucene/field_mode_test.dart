  ...
  group('Mode', () {
    ...
    group('apply', () {
      test('true sets setter to true', () {
        bool result = false;
        final f = lucene.Mode.auto('hd', false, (v) => result = v);
      ...
      test('false sets setter to false', () {
        bool result = true;
        final f = lucene.Mode.auto('hd', true, (v) => result = v);
      ...
      test('unrelated field does not call setter', () {
        bool called = false;
        final f = lucene.Mode.auto('hd', false, (_) => called = true);
    ...
    group('withCurrent', () {
      test('returns a Mode with the updated current value', () {
        final updated = field.withCurrent(true);
        expect(updated.current, isTrue);
        expect(updated.name, 'hd');
        expect(updated.defaultValue, isFalse);
      });
    });
  });
}
