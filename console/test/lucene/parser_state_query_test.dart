...
void main() {
  ...
  group('Query.consume', () {
    test('cursor == 0 returns Query with 0 remaining', () {
      final state = sm.Query(sm.Context(fields, -1, '', _noop, GlobalKey<SuggestionListState>()));
    ...
    test('cursor < lastOffset resets lastOffset to ctx.offset', () {
      final state = sm.Query(
        sm.Context(fields, 2, 'hel', _noop, GlobalKey<SuggestionListState>(), lastOffset: 5),
      );
    ...
    test('cursor == lastOffset returns Query with 0 remaining', () {
      final state = sm.Query(sm.Context(fields, -1, '', _noop, GlobalKey<SuggestionListState>(), lastOffset: 5));
    ...
    test('@ anchor offset is the position of @ in text', () {
      ...
      final state = sm.Query(sm.Context(fields, -1, '', _noop, GlobalKey<SuggestionListState>(), lastOffset: 6));
