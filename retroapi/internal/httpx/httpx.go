...
import (
	...
	"github.com/retrovibed/retroapi/internal/backoffx"
	...
	"github.com/retrovibed/retroapi/internal/stringsx"
...
func WriteJSONCode(resp http.ResponseWriter, code int, buffer *bytes.Buffer, context any) (err error) {
	buffer.Reset()
...
func NewUpload(fieldname, filename string, in io.Reader) (mime string, b io.ReadCloser, err error) {
	...
	go func() {
		defer func() {
			errorsx.Log(errorsx.Wrap(langx.FirstNonZero(mp.Close(), w.Close()), "failed to cleanup upload"))
		}()
