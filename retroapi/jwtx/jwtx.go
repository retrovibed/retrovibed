...
func ClaimsOptionExpiration(d time.Duration) Option {
	return func(rc *jwt.RegisteredClaims) {
		rc.ExpiresAt = jwt.NewNumericDate(rc.IssuedAt.Add(d))
	}
}
...
func Validate(jwtsecret SecretSource, encoded string, t jwt.Claims) error {
	...
	token, err := jwt.ParseWithClaims(string(encoded), t, func(token *jwt.Token) (any, error) {
		return jwtsecret(), nil
	}, jwt.WithValidMethods(algos))

	if err != nil {
		return errorsx.Wrapf(err, "unable to parse jwt token: %s - %T", string(encoded), t)
	}

	if !token.Valid {
		return errorsx.Errorf("invalid token %s - %v", string(encoded), t)
	}
